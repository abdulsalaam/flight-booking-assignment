...
function* fetchFlights() {
  try {
    const json = yield fetch('http://localhost:4000/api/flights')
      .then(response => response.json());
	  console.log('ddddddd:',json)
    yield put({ type: "GET_FLIGHTS_SUCCESS", json: transformFlightDates(json.flights) });
  } catch (e) {
    console.log('error', e);
